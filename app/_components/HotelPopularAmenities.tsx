...
function HotelPopularAmenities({ tags }: { tags: string[] }) {
  ...
  return (
    <div className="bg-white p-6 rounded-xl border border-gray-100  shadow-sm hover:shadow-md transition-all duration-300 group">
      <SectionTitle title={t("Popular Amenities")} underline />
      ...
      <div className="flex flex-wrap gap-3 mt-2">
        {tags.map((tagLabel) => {
          ...
          return (
            <div
              key={tagLabel}
              className="flex items-center gap-2 text-gray-700 bg-gray-50 hover:bg-gray-100 px-3 py-2 rounded-lg border border-gray-200 transition-all duration-200 hover:scale-[1.02] hover:shadow-sm cursor-default"
            >
              {Icon && (
                <Icon className="w-4 h-4 text-primary transition-transform duration-300 group-hover:rotate-12" />
              )}
